...
\caption{Reported Physical Activity vs.~Sleep Duration}
\end{figure}
...
the results. The assumption was that screen usage affects sleep. This is
because most devices emit blue light, which diminishes the amount of
...
\subsubsection{\texorpdfstring{\textbf{How does sleep duration vary with
BMI categories across different
genders?}}{How does sleep duration vary with BMI categories across different genders?}}\label{how-does-sleep-duration-vary-with-bmi-categories-across-different-genders}

```\{r\{r figure5-bmi-vs-sleep, fig.cap=``Sleep Duration vs Body Mass
Index (BMI) by Gender'', fig.width=8, fig.height=5, echo=FALSE,
warning=FALSE, message=FALSE\}

\section{============================================================}\label{section}

\section{Figure 5: Sleep Duration vs Body Mass Index (BMI) by
Gender}\label{figure-5-sleep-duration-vs-body-mass-index-bmi-by-gender}

\section{Script: Figure 5.R}\label{script-figure-5.r}

\section{Author: Alexandra Julka}\label{author-alexandra-julka}

\section{Modified: 2025-10-17}\label{modified-2025-10-17}

\section{============================================================}\label{section-1}

library(dplyr) library(ggplot2) library(forcats)

NHANESraw \textless- read.csv(``NHANESraw.csv'')

df\_bmi \textless- NHANESraw \%\textgreater\% select(SleepHrsNight,
BMI\_WHO, Gender) \%\textgreater\% filter(!is.na(SleepHrsNight),
!is.na(BMI\_WHO), !is.na(Gender)) \%\textgreater\% mutate( SleepHrsNight
= as.numeric(SleepHrsNight), BMI\_WHO = trimws(as.character(BMI\_WHO)),
\# use two-line axis labels for clarity BMI\_WHO = recode( BMI\_WHO,
``12.0\_18.5'' = ``Underweight\n(\textless18.5)'', ``18.5\_to\_24.9'' =
``Normal\n(18.5--24.9)'', ``25.0\_to\_29.9'' =
``Overweight\n(25--29.9)'', ``30.0\_plus'' = ``Obese\n(≥30)'' ),
BMI\_WHO = factor( BMI\_WHO, levels = c(
``Underweight\n(\textless18.5)'', ``Normal\n(18.5--24.9)'',
``Overweight\n(25--29.9)'', ``Obese\n(≥30)'' ), ordered = TRUE ), Gender
= factor(tolower(Gender), levels = c(``female'',``male''), labels =
c(``Female'',``Male'')) ) \%\textgreater\% filter(SleepHrsNight
\textgreater= 2, SleepHrsNight \textless= 12)

\section{Plot}\label{plot}

fig5\_clean \textless- ggplot(df\_bmi, aes(x = BMI\_WHO, y =
SleepHrsNight, fill = Gender)) + geom\_violin(trim = TRUE, alpha = 0.55,
color = ``grey40'', width = 0.85) + geom\_boxplot(width = 0.12,
outlier.shape = 16, outlier.size = 1.2, fill = ``white'', color =
``grey25'') + stat\_summary(fun = mean, geom = ``point'', shape = 21,
size = 2.3, fill = ``white'', color = ``black'') +
facet\_wrap(\textasciitilde{} Gender, nrow = 1) +
scale\_fill\_manual(values = c(``Female'' = ``\#F4A7B9'', ``Male'' =
``\#A7C7E7''), name = ``Gender'') + scale\_y\_continuous(breaks = 2:12,
limits = c(2, 12)) + labs( title = ``BMI vs Hours of Sleep'', subtitle =
``Violin = distribution; box = IQR; white dot = mean (NHANES
2009--2011)'', x = ``BMI Category (WHO Classification)'', y = ``Hours of
Sleep per Night'' ) + theme\_minimal(base\_size = 13) + theme(
panel.grid.minor = element\_blank(), plot.title.position = ``plot'',
strip.text = element\_text(face = ``bold'', size = 12), axis.text.x =
element\_text(size = 11, vjust = 1.2, lineheight = 1.1), axis.title.x =
element\_text(margin = margin(t = 10)), plot.margin = margin(10, 15, 15,
15) )
